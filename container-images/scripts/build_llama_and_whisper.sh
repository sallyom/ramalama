...
add_stream_repo() {
  local url="https://mirror.stream.centos.org/10-stream/$1/$uname_m/os/"
  local repo_name="centos-stream-10-stream-$(echo $1 | tr '[:upper:]' '[:lower:]')"

  cat > "/etc/yum.repos.d/${repo_name}.repo" << EOF
[${repo_name}]
name=CentOS Stream 10-stream - $1
baseurl=${url}
enabled=1
gpgcheck=0
EOF
...
dnf_install_mesa() {
  if [ "${ID}" = "fedora" ]; then
    ...
    for pkg in mesa-vulkan-drivers "${vulkan_rpms[@]}"; do
      dnf install -y "$pkg" || echo "Failed to install $pkg"
    done
    ...
    for pkg in "${alternative_pkgs[@]}"; do
      dnf install -y "$pkg" || echo "Failed to install alternative package $pkg"
    done
  fi
...
dnf_install_epel() {
  ...
  local url="https://dl.fedoraproject.org/pub/epel/epel-release-latest-10.noarch.rpm"
